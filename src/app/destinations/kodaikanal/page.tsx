...
/* ─── Kodaikanal Images ─────────────────────────────────────────────── */
const HERO_IMG = "https://images.unsplash.com/photo-1582510003544-4d00b7f74220?w=1800&q=85&fit=crop";
const GALLERY  = [
  { src:"https://images.unsplash.com/photo-1609766418204-94aae0ecfdfc?w=800&q=85&fit=crop", caption:"Kodai Lake" },
  { src:"https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=800&q=85&fit=crop", caption:"Coaker's Walk" },
  { src:"https://images.unsplash.com/photo-1469474968028-56623f02e42e?w=800&q=85&fit=crop", caption:"Pine Forest" },
  { src:"https://images.unsplash.com/photo-1501854140801-50d01698950b?w=800&q=85&fit=crop", caption:"Silver Cascade Falls" },
  { src:"https://images.unsplash.com/photo-1448375240586-882707db888b?w=800&q=85&fit=crop", caption:"Misty Valleys" },
  { src:"https://images.unsplash.com/photo-1441974231531-c6227db76b6e?w=800&q=85&fit=crop", caption:"Bryant Park" },
];

/* ─── Content Data ──────────────────────────────────────────────────── */
const HIGHLIGHTS = [
  { icon:"🌊", title:"Kodai Lake",          desc:"Star-shaped 60-acre lake at the heart of town — boat rides, cycling tracks and lakeside cafés await." },
  { icon:"🚶", title:"Coaker's Walk",        desc:"1.4 km scenic cliff-edge promenade with panoramic views of the Palani Hills and distant plains." },
  { icon:"🌲", title:"Pillar Rocks",         desc:"Three enormous granite pillars rising 122 m from the valley floor — dramatic and unforgettable." },
  { icon:"💧", title:"Silver Cascade Falls", desc:"75-ft roadside waterfall fed by Kodai Lake overflow; a classic stop on every Kodai itinerary." },
  { icon:"🌸", title:"Bryant Park",          desc:"Botanical garden with 325+ plant varieties, orchid house and a stunning annual flower show in May." },
  { icon:"🏔️", title:"Dolphin's Nose",      desc:"A flat projecting rock resembling a dolphin's snout — mesmerising views of Echo Valley and Shola." },
];
...
const PACKAGES = [
  ...
  },
  {
    ...
    features:[
      ...
      "Kurinji Andavar temple trek",
      "Vattakanal waterfall hike",
...
const ITINERARY = [
  {
    ...
    items:[
      "Pick-up from Dindigul / Madurai / Coimbatore",
      "Scenic ghat road drive through Palani Hills",
      "Check-in & freshen up at hotel",
      "Evening stroll along Kodai Lake",
      "Sunset at Coaker's Walk viewpoint",
  ...
  },
  {
    ...
    items:[
      "Early morning Bryant Park walk",
      "Silver Cascade Falls visit",
      "Bear Shola Falls short trek",
      "Rose Garden & Orchid House",
      "Chocolate factory tour & tasting",
      "Local market shopping on Anna Salai",
    ],
  },
  {
    day:3,
    title:"Peaks & Panoramas",
    items:[
      "Pillar Rocks — sunrise view",
      "Green Valley View (Suicide Point)",
      "Dolphin's Nose rock outcrop",
      "Pine Forest cycling / nature walk",
      "Kurinji Andavar Temple",
      "Bonfire evening at hotel",
    ],
  },
  {
    day:4,
    title:"Hidden Trails",
    items:[
      "Vattakanal waterfall trek (2 hrs)",
      "Pambar Falls exploration",
      "Berijam Lake (permit required)",
      "Star Gazing Point evening",
      "Traditional Tamil dinner experience",
    ],
  },
  {
    day:5,
    ...
    items:[
      ...
      "Last stop: Kodai Lake cycling",
      "Shopping — homemade chocolates, eucalyptus oil",
      "Drop to Dindigul / Madurai / Coimbatore",
    ],
  },
];

const BEST_TIME = [
  { month:"Apr – Jun", label:"Summer",    note:"Pleasant 8–20°C, best views, peak season",  active:true  },
  { month:"Jul – Sep", label:"Monsoon",   note:"Heavy mist & rain, very lush greenery",      active:false },
  { month:"Oct – Nov", label:"Post-rain", note:"Waterfalls full, clear skies, fewer crowds", active:true  },
  { month:"Dec – Mar", label:"Winter",    note:"Cold 5–15°C, foggy — romantic for couples",  active:true  },
];

const FAQS = [
  { q:"How to reach Kodaikanal?",             a:"Nearest railway station is Kodaikanal Road (80 km). Nearest airports are Madurai (120 km) and Coimbatore (175 km). We arrange comfortable cab transfers from all these points." },
  { q:"Is Kodaikanal suitable for families?", a:"Yes! Kodai Lake boating, Bryant Park, Silver Cascade and the chocolate factory are all family-friendly. The gentle weather and scenic roads make it ideal for all ages." },
  { q:"What is the best season to visit?",    a:"April–June and October–November are ideal. December–January is peak honeymoon season — foggy, cold and romantic. July–September is rainy but strikingly green." },
  { q:"Is Kodaikanal good for a honeymoon?",  a:"Absolutely. Misty mornings, lakeside dinners, pine forest walks and heritage cottages make Kodai one of South India's most romantic honeymoon destinations." },
  { q:"Do you need a permit for Berijam Lake?", a:"Yes, Berijam Lake requires a Forest Department permit obtained from the Kodaikanal Forest Office (near the lake). We assist our guests with permit arrangements in advance." },
];
...
function Breadcrumb() {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, fontFamily:FB, fontSize:11, fontWeight:600, color:"rgba(150,210,90,0.55)", letterSpacing:".04em" }}>
      ...
      <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><path d="M9 18l6-6-6-6"/></svg>
      <span style={{ color:C.gLeaf }}>Kodaikanal</span>
    </div>
  );
}
...
export default function KodaikanalPage() {
  ...
  // mounted state removed – animation handled via CSS
    ...
    <div style={{ fontFamily:FB, background:"#040E02", minHeight:"100vh", color:"#fff" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing:border-box; margin:0; padding:0; }
        ...
        @keyframes scaleIn   { from { transform: scale(1.07); } to { transform: scale(1); } }
      ...
      <div ref={heroRef} style={{ position:"relative", minHeight:"100vh", overflow:"hidden", display:"flex", flexDirection:"column" }}>
        <div style={{ position:"absolute", inset:0, zIndex:0, backgroundImage:`url(${HERO_IMG})`, backgroundSize:"cover", backgroundPosition:"center 40%", animation: "scaleIn 2s cubic-bezier(.22,1,.36,1) both" }}/>
        ...
        <div style={{ position:"relative", zIndex:10, maxWidth:maxW, margin:"0 auto", width:"100%", padding:`0 ${px}`, flex:1, display:"flex", flexDirection:"column", justifyContent:"center", paddingTop:isMobile?"90px":"80px", paddingBottom:isMobile?"64px":"80px" }}>
          ...
          <div style={{ display:"inline-flex", alignItems:"center", gap:8, marginBottom:20, animation:heroVis?"fadeUp .55s .05s ease both":"none" }}>
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"7px 16px", borderRadius:100, background:"rgba(107,189,40,0.12)", border:"1.5px solid rgba(107,189,40,0.28)", backdropFilter:"blur(12px)" }}>
              ...
              <span style={{ fontFamily:FB, fontSize:isXS?10:11, fontWeight:700, color:C.gLeaf, letterSpacing:".18em", textTransform:"uppercase" }}>Princess of Hills · Tamil Nadu</span>
            </div>
          </div>
          ...
          <h1 style={{ fontFamily:F, fontStyle:"italic", fontWeight:800, fontSize:isXS?"36px":isMobile?"50px":isTablet?"68px":"88px", lineHeight:.97, letterSpacing:"-.038em", color:"#fff", margin:"0 0 8px", textShadow:"0 4px 40px rgba(0,0,0,0.55)", animation:heroVis?"fadeUp .62s .10s ease both":"none" }}>
            <span style={{ background:`linear-gradient(125deg,${C.gLeaf} 0%,#C8F060 38%,${C.gLight} 65%,${C.gMid} 100%)`, backgroundSize:"200% auto", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", animation:"shimmer 4s linear infinite" }}>Kodaikanal</span>
          </h1>
          ...
          <p style={{ fontFamily:FB, fontSize:isXS?13:14, fontWeight:500, color:"rgba(185,228,130,0.60)", maxWidth:490, lineHeight:1.84, margin:"0 0 36px", animation:heroVis?"fadeUp .62s .18s ease both":"none" }}>
            Perched at 2,133m in the Palani Hills, Kodaikanal bewitches with its star-shaped lake, dense shola forests, cascading waterfalls and chocolate-scented lanes — a dreamer&apos;s refuge just 120 km from Madurai.
          </p>

          {/* Quick facts */}
          <div style={{ display:"flex", flexWrap:"wrap", gap:10, marginBottom:36, animation:heroVis?"fadeUp .62s .22s ease both":"none" }}>
            {[
              { icon:"🌡️", text:"8–20°C Avg"       },
              { icon:"📍", text:"2,133m altitude"   },
              { icon:"🚗", text:"120 km from MDU"   },
      ...
      <div style={{ background:"#040E02", padding:`72px ${px}`, maxWidth:maxW, margin:"0 auto" }}>
        <Section>
          <Eyebrow text="Why Visit Kodaikanal"/>
          ...
          <p style={{ fontFamily:FB, fontSize:isXS?13:14, color:"rgba(175,225,120,0.58)", maxWidth:520, lineHeight:1.80, marginBottom:40 }}>
            Six must-see experiences curated by our local experts — every one a reason to fall in love with Kodai.
          </p>
      ...
      <div style={{ padding:`72px ${px}`, maxWidth:maxW, margin:"0 auto" }}>
        <Section>
          ...
          <p style={{ fontFamily:FB, fontSize:14, color:"rgba(175,225,120,0.58)", maxWidth:480, lineHeight:1.80, marginBottom:36 }}>
            Based on our Classic Kodai — 3 Nights / 4 Days package. Fully customisable on request.
          </p>
      ...
      <div style={{ padding:`72px ${px}`, maxWidth:maxW, margin:"0 auto" }}>
        <Section>
          ...
          <h2 style={{ fontFamily:F, fontStyle:"italic", fontSize:isXS?"26px":isMobile?"32px":"42px", fontWeight:800, color:"#fff", marginBottom:36, letterSpacing:"-.03em" }}>
            When to Go to{" "}
            <span style={{ background:`linear-gradient(125deg,${C.gLeaf},#C8F060,${C.gMid})`, backgroundSize:"200% auto", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", animation:"shimmer 4s linear infinite" }}>Kodaikanal</span>
          </h2>
      ...
      <div id="enquiry" style={{ position:"relative", overflow:"hidden", padding:`80px ${px} 96px` }}>
        ...
        <Section style={{ position:"relative", zIndex:10, maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          ...
          <h2 style={{ fontFamily:F, fontStyle:"italic", fontSize:isXS?"28px":isMobile?"36px":"52px", fontWeight:800, color:"#fff", letterSpacing:"-.034em", lineHeight:1.04, marginBottom:16, textShadow:"0 4px 32px rgba(0,0,0,0.55)" }}>
            Ready for Your{" "}
            <span style={{ background:`linear-gradient(125deg,${C.gLeaf} 0%,#C8F060 45%,${C.gMid} 100%)`, backgroundSize:"200% auto", WebkitBackgroundClip:"text", WebkitTextFillColor:"transparent", backgroundClip:"text", animation:"shimmer 4s linear infinite" }}>Kodai Escape?</span>
          </h2>
          <p style={{ fontFamily:FB, fontSize:isXS?13:14.5, color:"rgba(180,225,130,0.62)", lineHeight:1.80, marginBottom:36 }}>
            Our travel experts will craft your personalised Kodaikanal itinerary — for free, with no advance payment required. Just reach out and we&apos;ll call you within 2 hours.
          </p>
          ...
          <div style={{ display:"flex", gap:12, justifyContent:"center", flexWrap:"wrap", marginBottom:28 }}>
            ...
            <a href="https://wa.me/919876543210?text=Hi! I want to plan a trip to Kodaikanal." target="_blank" rel="noreferrer" className="cta-btn" style={{ fontFamily:FB, fontSize:isXS?13:14, fontWeight:700, padding:isXS?"12px 20px":"15px 28px", borderRadius:14, border:"2px solid rgba(37,211,102,0.45)", background:"rgba(37,211,102,0.10)", color:"rgba(37,211,102,0.95)", cursor:"pointer", letterSpacing:".04em", textDecoration:"none", display:"inline-flex", alignItems:"center", gap:9, backdropFilter:"blur(10px)" }}>
